...
export function seedSopBody(): string {
	return [
		...
		"## File layout",
		"",
		"- 一条 SOP = 一个文件：全局写 `sop/<name>.md`（平铺，不要子目录）",
		"- 项目专属 SOP 写 `projects/<项目键>/<name>.md`（用 `sop_save` 的 `project=true`）",
		"  —— 项目键是归一化后的 origin URL（如 `git.woa.com/org/repo`），由工具自动推导，不要手填。",
		"  项目目录优先于全局加载，但**两边都不允许同名**：同名时先注册者静默胜出，是实测确认的坑",
		...
		"## Language（多语言约定）",
		"",
		"- **正文单语**：用作者最擅长的语言写，不强制中文或英文。一条 SOP 只写一种语言，",
		"  不要中英混排正文（混排会让后来的读者和检索都变差）",
		"- **triggers 语言混合**：中英关键词都塞进去，检索是它的本职；中文提问和英文提问都应该能命中",
		"- **description 可选双语**：需要双语时写成 `英文 | 中文`，用半角竖线分隔。",
		"  pi 读整条 description 做技能发现；MANIFEST 只取竖线前的英文段（否则双语串撑爆表格）。",
		"  不写竖线 = 单语，完全合法",
		"- 不做 `.zh-CN.md` / `.en.md` 语言后缀方案，也不按语言过滤 skill",
		"",
		...
		"description: USE FOR deploying MySQL replicas, setting up replication, GTID config | 用于部署 MySQL 从库、配置复制与 GTID",
		...
		"| 字段 | 说明 |",
		"|---|---|",
		"| `name` | 与文件名一致，小写连字符；全库（含所有 projects/）内必须唯一 |",
		"| `description` | **必需**，决定 pi 何时加载这条 skill。写清「什么时候用」，不要只写「是什么」；可选用 `英文 | 中文` 双语 |",
		...
		"- 只对当前项目成立的流程放 `projects/<项目键>/`（`sop_save` 传 `project=true`），",
		"  跨项目复用的放全局 `sop/`；说不清就默认全局（不传 `project` 就是全局）",
		"",
	].join("\n");
}
